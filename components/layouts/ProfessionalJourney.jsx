...
const JOURNEY = [
  {
    ...
    description:
      "Applying theoretical knowledge to real-world business problems through internships.",
    icon: Briefcase,
    color: "text-blue-400",
    items: [
      {
  ...
  },
  {
    phase: "Foundation",
    description:
      "Building the core pillars of Computer Science and Data Analytics.",
    icon: GraduationCap,
    ...
    >
      ...
      >
        ...
        <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
          From classroom fundamentals to building and shipping projects.
        </p>
      </motion.div>
              ...
              <div key={index} className="relative">
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center gap-6 mb-12">
                  ...
                  <div className="pl-20 md:pl-12 md:w-1/2">
                    <h3
                      className={`text-2xl font-bold ${phase.color} md:hidden`}
                    >
                      {phase.phase}
                    </h3>
                    <p className="text-muted-foreground md:hidden text-sm mb-2">
                      {phase.period}
                    </p>
                  ...
                  {phase.items.map((item, i) => (
                    ...
                    >
                      ...
                      <div className="md:w-1/2 flex justify-end md:px-12 pl-20">
                        <div
                          className={`w-full p-6 rounded-xl border border-border bg-card/50 backdrop-blur-sm shadow-sm hover:shadow-md transition-all duration-300 group relative overflow-hidden ${index % 2 !== 0 && "md:text-left"}`}
                        >
                          <div className="flex flex-col gap-2 mb-4">
                            ...
                            <span
                              className={`text-sm font-medium ${phase.color}`}
                            >
                              {item.organization}
                            </span>
                          </div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {item.tags?.map((tag, t) => (
                              <Badge
                                key={t}
                                variant="secondary"
                                className="px-2 py-0.5 text-[10px] uppercase tracking-wider bg-secondary/30 text-secondary-foreground/70"
                              >
                                {tag}
                              </Badge>
                            ))}
